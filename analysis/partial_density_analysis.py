...
DEPOSITED = {
    ...
    45: os.path.join(DATA_PATH, "ip_45n/{0}_density.xvg"),
...
TITLE_MAP = {15:"2.0 wt%",40:"5.3 wt%",45:"6 wt%",108:"14.1 wt%",209:"26.3 wt%"}
def parse_xvg(xvg):
    with open(xvg) as fh:
...
def plot_densities(data):
    ...
    for i, (n_irppy, densities) in enumerate(sorted(data.items())):
        ...
        for j, (species, (z, density)) in enumerate(sorted(densities.items())):
            ...
            plot(ax, z, density, color="k", text=TITLE_MAP[n_irppy], xlabel=x_label, ylabel=y_label, linewidth=line_width[j], dashes=line_styles[j])
...
def plot_density(ax, z, density, n_irppy, x_label, y_label, linewidth, dashes):
    plot(ax, z, density, color="k",
        text=TITLE_MAP[n_irppy],
        text_location=(0.87, 0.9),
        xlabel=x_label,
        ylabel=y_label,
        zorder=2,
        linewidth=linewidth,
...
def single_frame_analysis_for_all():
    density_data = {}
    for n_irppy in (15, 40, 45, 108, 209):
        density_data[n_irppy] = calculate_density_data(n_irppy)
    plot_densities(density_data)

if __name__=="__main__":
    #single_frame_analysis_for_all()
    densities = calculate_density_data(45)
    fig = create_figure((4,3))
    ax = add_axis_to_figure(fig)
    x_label = "z (nm)"
    y_label = "density (g cm$^{-3}$)"
    for j, (species, (z, density)) in enumerate(sorted(densities.items())):
        plot_density(ax, z, density, 45, x_label, y_label, linewidth=line_width[j], dashes=line_styles[j])
    fig.tight_layout()
    save_figure(fig, "./partial_densitie_6wt%", image_format=IMAGE_FORMAT)